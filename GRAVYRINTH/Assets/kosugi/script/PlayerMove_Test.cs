/**==========================================================================*/
/**
 * プレイヤーの挙動テスト用個人的スクリプト
 * 
 * 騙し騙しの処理ばかりなので早急に精度を上げるように！
 *
 * 作成者：小杉   作成日：16/10/28
/**==========================================================================*/
...
public class PlayerMove_Test : MonoBehaviour   //当たったかどうか付のRaycastHit
{
    ...
    //現在の前方向
    private Vector3 m_Front = Vector3.forward;
    //現在の右方向
    ...
    private void Move()
    {
        ...
        {
            //壁蹴り用タイマー開始
            m_WallKickTimer += Time.deltaTime;
        }
    ...
    private bool Collision()
    {
        ...
        Ray ray_front = new Ray(rayPos, m_Front);
        Ray ray_left = new Ray(rayPos, m_Front - m_Right);
        Ray ray_right = new Ray(rayPos, m_Front + m_Right);
    ...
    {
        if (m_WallKickTimer > 0.5f)
        {
            if (Vector3.Angle(m_Up, m_WallHitInfoFront.hit.normal) == 90)
            {
                print("!");
                rb.AddForce((-rb.velocity + -GetDown()) * m_GravityPower * 0.55f);
                if (Input.GetKeyDown(KeyCode.Space))
                {
                    //力を加えてジャンプ
                    rb.AddForce(m_Up * m_JumpPower - m_Front * m_JumpPower);
