...
public class test0112 : MonoBehaviour
{
    /*==所持コンポーネント==*/
    ...
    private void CheckGroundHit()
    {
        ...
        //レイをデバッグ表示
        Debug.DrawRay(rayPos, GetDown() * m_RayLength, Color.white, 1.0f, false);
    ...
    public void WallKick()
    {
        ...
        if ((180 - m_WallKickAbleAngle / 2 < wallAngle && wallAngle < 180 + m_WallKickAbleAngle / 2))
        {
            ...
            Vector3 rotate = new Vector3(tr.localRotation.x, tr.localRotation.y, tr.localRotation.z);
    ...
    public void WallHold()
    {
        ...
        //登り方を詰める→前おしっぱで上れる 原因不明
        //向き等を調整→ベクトルで調整？壁側のベクトルどうとる？
        //硬直を修正→たぶんおｋ
    }
    /// <summary>
    /// 崖のぼり
    ...
    public void Clamb()
    {
        //Vector3 rayPos = tr.position + tr.up / 5f;
        //Ray ray = new Ray(rayPos, tr.forward);

        //RaycastHit hit_knee;
        //RayHitInfo m_WallHitKnee;

        ////[IgnoredObj]レイヤー以外と判定させる
        //int layermask = ~((1 << LayerMask.NameToLayer("IgnoredObj")) + (1 << LayerMask.NameToLayer("WallHold")));
        //m_WallHitKnee.isHit = Physics.Raycast(ray, out hit_knee, 0.25f, layermask);

        //m_WallHitKnee.hit = hit_knee;

        ////レイをデバッグ表示
        //Debug.DrawRay(rayPos, tr.forward, Color.white, 0.25f, false);

        //if (!m_WallHitInfoFront.isHit && m_WallHitKnee.isHit)
        //{
        //    // アニメーション
        //    anm.SetTrigger("ClambSmall");
        //}
    }
}